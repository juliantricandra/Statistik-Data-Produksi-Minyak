...
with st.container() :
    ...
    df_barchart = alt.Chart(df_b).mark_bar().encode(tooltip=['produksi','tahun','region','sub_region','alpha3_negara'],
    x=alt.X('produksi:N', axis=alt.Axis(title='Jumlah Produksi')),
    y=alt.Y('kode_negara', axis=alt.Axis(title='Negara'),sort=alt.EncodingSortField(field='produksi', order='ascending',op='sum')))
